...
# Main class
######################################################################
class MachineLearningNameEthnicityProjectBinaryClass(ml_model.MachineLearning, 
	...
	def __init__(self, control_panel):
		...
		assert None not in [control_panel['save_result_switch'], control_panel['use_subsampled_df_switch'],
							...
							control_panel['ml_process_on_ext_data_switch'],	control_panel['binary_target_label']
		...
		# Reassigning
		self.control_panel['binary_target_label'] = self.control_panel['binary_target_label'].lower()	
		# Decision boundary threshold to classify binary ethnicity target label
	...
	def data_prep(self):
		...
		self.feature_list_all = self.feature_list_num + self.feature_list_cat + self.feature_list_liststring + self.feature_list_liststring1letter

		# Declare predicted outcome label.
		label_tree = self.label_tree()
		if self.control_panel['binary_target_label'] in label_tree['ab_groups']:
			self.pred_outcome = var_tree['ethnic']['final_cat'][1]
		elif self.control_panel['binary_target_label'] in label_tree['fn_tribal_groups']:
			self.pred_outcome = var_tree['ethnic']['final_cat'][2]
		elif self.control_panel['binary_target_label'] in label_tree['fn_language_groups']:
			self.pred_outcome = var_tree['ethnic']['final_cat'][3]
		elif self.control_panel['binary_target_label'] in label_tree['main_groups']: 
			self.pred_outcome = var_tree['ethnic']['final_cat'][0]
		else:
			raise ValueError('Error: Incorrect value for control_panel["binary_target_label"].')

		# Binarize outcome in df
		self.pred_outcome_vFinal = self.pred_outcome+'_VFINAL'
		self.df[self.pred_outcome_vFinal] = np.where(self.df[self.pred_outcome]==self.control_panel['binary_target_label'], 
			self.control_panel['binary_target_label'], '~{}'.format(self.control_panel['binary_target_label']))
		assert len(self.df[self.df[self.pred_outcome_vFinal]==self.control_panel['binary_target_label']])>0, \
			'Error: Count of target label is 0.'
		# Data split
	...
	def feature_selection_result(self):
		if self.control_panel['feature_selection_switch']:
			...
			if self.control_panel['save_result_switch']:
				with open('{}{}{}_{}_SubsampleIs{}_ScoreIs{}_EthIs{}.txt'.format(self.result_dir, 'BinaryClass_FeatureSelection_', 
					...
					str(self.control_panel['use_subsampled_df_switch']), self.control_panel['eval_score'].capitalize().replace(
						' ', ''), self.control_panel['binary_target_label'].capitalize()), 'w') as f:
	...
	def cross_validation_result(self):
		if self.control_panel['cross_validation_switch']:
			...
			if self.control_panel['save_result_switch']:
				...
				with open('{}{}{}_{}_SubsampleIs{}_ScoreIs{}_EthIs{}_Take{}.txt'.format(self.result_dir, 'BinaryClass_CV_BestParameters_', 
					self.control_panel['ml_algo_param_grid'][0], self.control_panel['use_feature_set'],
					str(self.control_panel['use_subsampled_df_switch']), self.control_panel['eval_score'].capitalize().replace(' ', ''), 
					self.control_panel['binary_target_label'].capitalize(),	self.control_panel['cross_validation_repeat']), 'w') as f:
					print('>> Setting:\n', self.control_panel, file=f)
	...
	def ml_process(self):
		...
		if (self.control_panel['ml_process_on_test_data_switch']|self.control_panel['ml_process_on_ext_data_switch']):
			...
			if self.control_panel['save_result_switch']:
				fname = '{}{}{}_{}_SubsampleIs{}_EthIs{}.pickle'.format(self.result_dir, 'BinaryClass_', 
					self.control_panel['ml_algo'][0], self.control_panel['use_feature_set'], 
					str(self.control_panel['use_subsampled_df_switch']), 
					self.control_panel['binary_target_label'].capitalize())
				pickle.dump(self.pipe, open(fname, 'wb'))

	def ml_process_result_on_test_data(self, _save_df_with_predicted_labels=True):
		if self.control_panel['ml_process_on_test_data_switch']:
			if self.control_panel['use_subsampled_df_switch']:
				print('WARNING: Full data is not being used currently.\n')
			target_labels = [self.control_panel['binary_target_label'], 
				'~{}'.format(self.control_panel['binary_target_label'])]
			predicted_labels_bool = (self.pipe.predict_proba(self.X_test)[:, 0] > self.d_threshold).astype(bool)
			predicted_labels = np.vectorize(bool_to_str)(predicted_labels_bool, target_labels[0], target_labels[1]).astype(str)
			#predicted_labels = self.pipe.predict(self.X_test) # using default decision threshold as > 0.5
			predicted_prob, predicted_prob_inverted = self.pipe.predict_proba(self.X_test)[:, 1], \
				self.pipe.predict_proba(self.X_test)[:, 0]
			...
			print('>> Custom classification report:\n', self.custom_classification_report(self.y_test, predicted_labels), '\n')
			print('>> Custom scores:\n', self.auc_roc(self.y_test, predicted_prob), '\n', 
				self.avg_precision(self.y_test, predicted_prob_inverted, target_labels[0]), '\n')
			print('//////////////////////\n'*2)

			if self.control_panel['save_result_switch']:
				with open('{}{}{}_{}_SubsampleIs{}_EthIs{}.txt'.format(self.result_dir, 'BinaryClass_ML_Process_OnTestData_', 
					self.control_panel['ml_algo'][0], self.control_panel['use_feature_set'],
					str(self.control_panel['use_subsampled_df_switch']), self.control_panel['binary_target_label'].capitalize()
					), 'w') as f:
			...
			if _save_df_with_predicted_labels:
				df_y_test = pd.DataFrame(self.y_test)
				df_y_test['ETHNICITY_PRED'] = predicted_labels
				df_X_test = self.X_test.copy()
				df_full_test = df_y_test.merge(df_X_test, left_index=True, right_index=True)
				df_full_test.to_csv('{}{}{}_{}_SubsampleIs{}_EthIs{}.csv'.format(self.result_dir, 
					'BinaryClass_ML_Process_DFwPredLabels_OnTestData_', self.control_panel['ml_algo'][0], self.control_panel['use_feature_set'],
					str(self.control_panel['use_subsampled_df_switch']), self.control_panel['binary_target_label'].capitalize()
					), sep=',', index=False)
	...
	def ml_process_result_on_training_data(self):
		if self.control_panel['ml_process_on_training_data_switch']:
			...
			target_labels = [self.control_panel['binary_target_label'], 
				'~{}'.format(self.control_panel['binary_target_label'])]
			predicted_labels_bool = (self.pipe.predict_proba(self.X_train)[:, 0] > self.d_threshold).astype(bool)
			predicted_labels = np.vectorize(bool_to_str)(predicted_labels_bool, target_labels[0], target_labels[1]).astype(str)
			#predicted_labels = self.pipe.predict(self.X_train) # using default decision threshold as > 0.5
			predicted_prob, predicted_prob_inverted = self.pipe.predict_proba(self.X_train)[:, 1], \
				self.pipe.predict_proba(self.X_train)[:, 0]
			...
			print('>> Custom classification report:\n', self.custom_classification_report(self.y_train, predicted_labels), '\n')
			print('>> Custom scores:\n', self.auc_roc(self.y_train, predicted_prob), '\n', 
				self.avg_precision(self.y_train, predicted_prob_inverted, target_labels[0]), '\n')
			print('//////////////////////\n'*2)

			if self.control_panel['save_result_switch']:
				with open('{}{}{}_{}_SubsampleIs{}_EthIs{}.txt'.format(self.result_dir, 'BinaryClass_ML_Process_OnTrainingData_', 
					self.control_panel['ml_algo'][0], self.control_panel['use_feature_set'],
					str(self.control_panel['use_subsampled_df_switch']), self.control_panel['binary_target_label'].capitalize()
					), 'w') as f:
					...
					print('>> Custom classification report:\n', self.custom_classification_report(self.y_train, 
						predicted_labels), '\n', file=f)
					print('>> Custom scores:\n', self.auc_roc(self.y_train, predicted_prob), '\n', 
						self.avg_precision(self.y_train, predicted_prob_inverted, target_labels[0]), '\n', file=f)
					print('//////////////////////\n'*2, file=f)
	...
	def ml_process_result_on_external_data(self):
		if self.control_panel['ml_process_on_ext_data_switch']:
			...
			self.df_external_processed[self.pred_outcome_vFinal] = np.where(
				self.df_external_processed['ETHNICITY_RECAT'].str.lower()==self.control_panel['binary_target_label'], 
				self.control_panel['binary_target_label'], '~{}'.format(self.control_panel['binary_target_label']))
			# Data split
			...
			target_labels = [self.control_panel['binary_target_label'], 
				'~{}'.format(self.control_panel['binary_target_label'])]		
			predicted_labels_bool = (self.pipe.predict_proba(self.X_ext_test)[:, 0] > self.d_threshold).astype(bool)
			predicted_labels = np.vectorize(bool_to_str)(predicted_labels_bool, target_labels[0], target_labels[1]).astype(str)
			#predicted_labels = self.pipe.predict(self.X_ext_test) # using default decision threshold as > 0.5
			predicted_prob, predicted_prob_inverted = self.pipe.predict_proba(self.X_ext_test)[:, 1], \
				self.pipe.predict_proba(self.X_ext_test)[:, 0]
			...
			print('>> Classification report:\n', classification_report(self.y_ext_test, predicted_labels, 
				target_names=target_labels))
			print('>> Custom classification report:\n', self.custom_classification_report(self.y_ext_test, 
				predicted_labels), '\n')
			print('>> Custom scores:\n', self.auc_roc(self.y_ext_test, predicted_prob), '\n', 
				self.avg_precision(self.y_ext_test, predicted_prob_inverted, target_labels[0]), '\n')
			print('//////////////////////\n'*2)

			if self.control_panel['save_result_switch']:
				with open('{}{}{}_{}_SubsampleIs{}_EthIs{}.txt'.format(self.result_dir, 'BinaryClass_ML_Process_OnExtData_', 
					self.control_panel['ml_algo'][0], self.control_panel['use_feature_set'],
					str(self.control_panel['use_subsampled_df_switch']), self.control_panel['binary_target_label'].capitalize()
					), 'w') as f:
...
def bool_to_str(val, true_as, false_as):
	if val == True:
		return true_as
	elif val == False:
		return false_as
	else: 
		raise ValidationError(f'{val} is not valid value.')